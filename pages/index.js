...
function gCls(key, val) {
  const neg = ['위험','불안정','축소','하락','약함','악화','주의','저조','부진','감소','침체','악성','불량','불가','실패','손실']
  const pos = ['안정','건강','확대','상승','강함','확장','낮음','활발','높음','우수','양호','성장','흑자','회복','증가','호조','긍정','성공','강세','선도','활성']
  if (key === 'risk' || key === 'financial') {
    if (val.includes('낮음')) return 'pos'
    if (val.includes('높음')) return 'neg'
    if (val.includes('위험') || val.includes('높음')) return 'neg'
  }
...
export default function Home() {
  ...
  const [loadingPhase, setLoadingPhase] = useState('') // checking | collecting | analyzing
  ...
  async function doAnalyze(n, manualData = null) {
    ...
    setProgress(15)
    setStatus('자료를 수집 중입니다...')
    setResult(null)
    try {
      setProgress(40)
      setLoadingPhase('analyzing')
      setStatus('자료를 분석 중입니다...')
              ...
              <div style={{
                ...
                <div style={{ position:'relative', width:56, height:56 }}>
                  <svg viewBox="0 0 56 56" style={{ width:56, height:56, animation:'spin 1.2s linear infinite' }}>
                    ...
                    <circle cx="28" cy="28" r="22" fill="none" stroke="#0f172a" strokeWidth="4"
                ...
                <div style={{ textAlign:'center' }}>
                  <p style={{ fontSize:14, fontWeight:700, color:'#0f172a', marginBottom:6 }}>
                    {loadingPhase === 'checking'   && '기업 정보를 확인하고 있습니다...'}
                    {loadingPhase === 'collecting' && '자료를 수집 중입니다...'}
                    {loadingPhase === 'analyzing'  && '자료를 분석 중입니다...'}
                  </p>
                  <p style={{ fontSize:12, color:'#9ca3af' }}>
                    {loadingPhase === 'checking'   && 'DART · SEC EDGAR 조회 중'}
                    {loadingPhase === 'collecting' && '네이버 뉴스 · Google News · DART · SEC · 커뮤니티'}
                    {loadingPhase === 'analyzing'  && 'Claude AI가 수집된 자료를 분석하고 있습니다'}
                  </p>
                </div>
                {/* 진행 바 */}
                <div style={{ width:280, height:3, background:'#e5e7eb', borderRadius:2, overflow:'hidden' }}>
                  <div style={{
                    height:'100%', borderRadius:2, background:'#0f172a',
                    width: loadingPhase === 'checking' ? '20%' : loadingPhase === 'collecting' ? '50%' : '80%',
                        ...
                        {items.map((item, i) => (
                          <div key={i} style={{ fontSize:12, color:'#0f172a', lineHeight:1.6 }}>
                            ...
                            <a
                              ...
                              onClick={e => { if(item.source_url){ e.preventDefault(); window.open(item.source_url,'_blank','noopener,noreferrer') } }}
                              style={{ fontSize:10, fontWeight:700, color:'#6366f1', marginLeft:4, cursor:'pointer' }}
                            >[{item.source}{item.date ? ', '+item.date : ''}]</a>
                          </div>
                        ))}
                      </div>
